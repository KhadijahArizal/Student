...
class Data extends ChangeNotifier {
  ...
  TextEditingController excontact =
      TextEditingController(text: 'Not Assign Yet');

  //Company Details
  String selectedFile = '';
  TextEditingController offerletter = TextEditingController();
  ...
  TextEditingController statusComp = TextEditingController();
  DateTime? startDate;
  DateTime? endDate;
  ...
  TextEditingController feedbackF = TextEditingController();
  TextEditingController statusF = TextEditingController();
  ...
  TextEditingController titleM = TextEditingController();
  TextEditingController statusM = TextEditingController();
  TextEditingController monthlyReportLink = TextEditingController();
  ...
  DateTime? clEndDate;
  void clCalculateDuration(BuildContext context) {
    if (clStartDate != null && clEndDate != null) {
      if (clEndDate!.isBefore(clStartDate!)) {
        clShowErrorDialog(context, 'Invalid date range');
      } else {
        Duration difference = clEndDate!.difference(clStartDate!);
        int months = (difference.inDays / 30).ceil(); // Use ceil to round up
        clDuration.text = months.toString(); // Update clDuration controller

        if (months < 6) {
          clShowErrorDialog(context, 'Duration must be at least 6 months.');
        }
      }
    }
  }
  void clShowErrorDialog(BuildContext context, String message) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
  ...
  TextEditingController grd = TextEditingController();
  TextEditingController prt = TextEditingController();
  TextEditingController cnfrm = TextEditingController();
  TextEditingController comment = TextEditingController();
  TextEditingController Iapstatus = TextEditingController();
}
