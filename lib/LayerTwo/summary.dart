...
class _SummaryState extends State<Summary> {
  ...
  late DatabaseReference _coorRef;
  late Future<List<SvExEmailData>> _userSvExFuture;

  int _currentIndex = 0;

  Future<void> onTabTapped(int index) async {
    await _statusManagement.fetchStudentStatus();
    setState(() {
      _currentIndex = index;
      if (index == 0) {
  ...
  void initState() {
    ...
    if (user != null) {
      ...
      _coorRef = FirebaseDatabase.instance
          .ref('Coordinator')
          .child('Coordinator Details');
      _userSvExFuture = _fetchSvData();
    }
  }
      ...
      if (companyData != null) {
        companyData.forEach((key, value) {
          if (value is Map<dynamic, dynamic> && key == userId) {
            ...
            String status = value['Status'] ?? 'Inactive';
  ...
  Future<List<SvExEmailData>> _fetchSvData() async {
    List<SvExEmailData> userDataList = [];
    try {
      ...
      DataSnapshot coorSnapshot =
          await _coorRef.once().then((event) => event.snapshot);
      ...
      Map<dynamic, dynamic>? coorData =
          coorSnapshot.value as Map<dynamic, dynamic>?;
      print('coorData: $coorData');

      if (svData != null && exData != null) {
        exData.forEach((key, value) {
          if (value is Map<dynamic, dynamic> &&
              svData.containsKey(key) && exData.containsKey(key) &&
              key == userId) {
            String svEmail = svData[key]['Email'] ?? '';
            String exEmail = exData[key]['ExaminerEmail'] ?? '';
            String coorEmail = coorData?['Email'] ?? '';

            SvExEmailData userSv = SvExEmailData(
              userId: userId,
              svEmail: svEmail,
              exEmail: exEmail,
              coorEmail: coorEmail,
            );
            userDataList.add(userSv);
          }
        });
      }
    } catch (e) {
  ...
  void fetchUserEmail(String userId) async {
    try {
      ...
      if (user != null) {
        ...
        User? userWithDetails = await FirebaseAuth.instance
            .userChanges()
            .firstWhere((user) => user?.uid == userId);

        // Now, you can access the email address of the user
        String? userEmail = userWithDetails?.email;
  ...
  void _showOptions(BuildContext context, SvExEmailData? usersv) {
  ...
  void _sendEmail(String recipientEmail) async {
    final Uri emailLaunchUri = Uri(
      ...
      path: recipientEmail,
      queryParameters: {
        'subject': 'ASKING ABOUT INTERNSHIP',
        'body': 'Dear Mr./Ms./Dr.\n',
      },
    );
...
*/
      ...
      floatingActionButton: FutureBuilder<List<SvExEmailData>>(
        future: _fetchSvData(),
        builder: (context, snapshot) {
...
class SvExEmailData {
  final String userId;
  final String exEmail, svEmail, coorEmail;

  SvExEmailData({
    required this.exEmail,
    required this.svEmail,
    required this.coorEmail,
    required this.userId,
  });
}
